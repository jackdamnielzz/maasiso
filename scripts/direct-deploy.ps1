...
"@
    ...
    # Also create a .env.local file to override any other settings
    Set-Content -Path (Join-Path $tempDir ".env.local") -Value $envContent -NoNewline -Encoding UTF8
    
    # Verify environment file content
    Write-Host "Environment file content:"
    Get-Content (Join-Path $tempDir ".env.production")
