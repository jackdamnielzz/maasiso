...
- **Contact Form Email: Resend SMTP**: 100% ✅

## Contact Form Email: Resend SMTP (2026-02-17) ✅

Switched contact form email from Office 365 to **Resend** SMTP. Office 365 blocked basic auth due to MFA/Security Defaults. Resend uses `smtp.resend.com:465` (SSL) with API key auth. Domain `maasiso.nl` is verified in Resend. Removed the `tls: { ciphers: 'SSLv3' }` workaround that was needed for O365.

**Files changed:**
- [`app/api/contact/route.ts`](app/api/contact/route.ts:33) — Updated defaults to Resend, removed TLS cipher hack
- [`.env.example`](.env.example:12) — Resend settings template
