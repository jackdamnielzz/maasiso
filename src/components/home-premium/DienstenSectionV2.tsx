...
import { extraDiensten } from "@/components/home-v2/data";
...
interface ExtraDienstStijl {
  readonly hoverBorder: string;
  readonly balk: string;
  readonly icoonVlak: string;
  readonly link: string;
  readonly icoon: ReactNode;
}

/** Visuele stijlen per extra dienst, in dezelfde volgorde als extraDiensten in data.ts. */
const extraDienstStijlen: readonly ExtraDienstStijl[] = [
  {
    hoverBorder: "hover:border-violet-300",
    balk: "from-violet-500 to-purple-600",
    icoonVlak: "bg-violet-50 text-violet-600",
  ...
  },
  {
    ...
    balk: "from-teal-500 to-cyan-600",
    icoonVlak: "bg-teal-50 text-teal-600",
    link: "text-teal-600",
    icoon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  },
  {
    ...
    balk: "from-orange-500 to-red-500",
    icoonVlak: "bg-orange-50 text-orange-600",
...
export function DienstenSectionV2({ dienstentabel }: DienstenSectionV2Props) {
  return (
    <article id="diensten">
      ...
      {/* Extra diensten — inhoud, links en labels 1-op-1 uit extraDiensten in data.ts */}
      <div className="mt-14 grid gap-6 md:grid-cols-3">
        {extraDiensten.map((dienst, index) => {
          const stijl = extraDienstStijlen[index] ?? extraDienstStijlen[0];
          return (
            <div
              key={dienst.titel}
              className={`group relative overflow-hidden rounded-2xl border border-[#d7e1ee] bg-white p-7 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${stijl.hoverBorder}`}
            >
              <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${stijl.balk}`} />
              <div className={`inline-flex rounded-xl p-3 ${stijl.icoonVlak}`}>
                {stijl.icoon}
              </div>
              <h3 className="mt-4 text-lg font-bold text-[#091E42]">{dienst.titel}</h3>
              <p className="mt-3 text-sm leading-relaxed text-gray-600">{dienst.beschrijving}</p>
              {dienst.feit ? (
                <p className="mt-3 text-sm leading-relaxed text-gray-600">{dienst.feit}</p>
              ) : null}
              <Link
                href={dienst.href}
                className={`mt-5 inline-flex items-center gap-1 text-sm font-semibold hover:underline ${stijl.link}`}
              >
                {dienst.linkLabel}
