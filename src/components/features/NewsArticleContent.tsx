...
const NewsArticleContent: React.FC<NewsArticleContentProps> = ({ article }) => {
  ...
  }, [article.slug, article.categories]);
  const {
    title,
    content,
    author,
    ...
    <ErrorBoundary>
      <div className="max-w-6xl mx-auto px-8 py-12 flex flex-col lg:flex-row gap-12">
        ...
        <header className="mb-16">
          <h1 className="text-[2.75rem] font-semibold text-[#091E42] mb-8 leading-[1.2]">
            {title}
          </h1>
          <div className="flex flex-wrap items-center text-[15px] text-[#6B778C] space-x-2">
            ...
            {categories && categories.length > 0 && (
              <>
                <span className="text-[#6B778C]/60 mx-1">•</span>
                <span>
                  {categories[0].name}
                </span>
              </>
        ...
        <div className="prose prose-lg max-w-none text-[#42526E]
            ...
            {relatedArticles.map(article => (
              ...
              >
                {article.categories && article.categories.length > 0 && (
                  <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-[#0052CC]/10 text-[#0052CC] mb-4">
